...
from ScatterWriter import ScatterWriter
...
#
    ...
    alldata = CD.ComputeCycleTime(Start_Time,datalist, ismajor) # returns a list of tuples: epochtime,cum,red,green,yellow
    cycledata = alldata[0]

    # Mutate
    #datalist_m = MT.ShortenRed(datalist)

    #alldata = CD.ComputeCycleTime(Start_Time,datalist_m, ismajor) # returns a list of tuples: epochtime,cum,red,green,yellow
    #cycledata_m = alldata[0]
    ...
    shortestredgreen = alldata[5]

    # Mutate

    #datalist2_m = MT.ShortenRed(datalist2)
    datalist2_m = MT.ShortenGreen(datalist2,shortestredgreen)
   

    alldata = CD.ComputeCycleTime(Start_Time,datalist2_m,False) # returns a list of tuples: epochtime,cum,red,green,yellow,metadata, indicator
    cycledata2_m = alldata[0]

    DS.GetDistribution(name, cycledata2, RT.minor)

    SR = ScatterWriter

    # write regular scatter
    #SR.WriteScatter(name, cycledata, cycledata2)
    #write normal scatter
    SR.WriteNormalScatter(name, cycledata, cycledata2_m)
...
# generate plot lines from indicators
    lines = BuildMultiIndicatorData(name, datalist, datalist2_m, datalist3)
# 
