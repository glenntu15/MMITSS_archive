...
QString DataManager::getMetadata(QString graphName, int pointid, double xp, double yp)
{
    QString datastring = "";
    if (numMetaCollumns == 0)
        return datastring;
    ...
    int pid = -1;
    int npts = pdata[index].length();
    double d1, d2;
    for (int i = 0;i<npts;i++) {
        d1 = abs(pdata[index][i].x - xp);
        if (d1 < .0001) {
            d2 = abs(pdata[index][i].y - yp);
            if (d2 < .0001) {
                pid = i;
                break;
            }
    ...
    if (pid == -1) {
        ...
        pairdata pd = pdata[index][pid];
        //int mdid = pd.indx;
        datastring = metadata[index][pid];
    }
